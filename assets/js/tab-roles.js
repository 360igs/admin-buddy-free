...
( function () {
    ...
    // Styled-select (.ab-dropdown--select) helpers. The hidden input still
    // exposes .value, but options live as <li class="ab-dropdown__option"> -
    // these helpers translate the operations the JS used to do on a native
    // <select> (option:checked, option[value=x], etc.) to the new DOM.
    function ddWrap(input){ return input ? input.closest('.ab-dropdown') : null; }
    function ddSelectedLabel(input){
        var dd = ddWrap(input); if(!dd){ return ''; }
        var sel = dd.querySelector('.ab-dropdown__option.is-selected');
        return sel ? sel.textContent.trim() : '';
    }
    function ddSetValue(input, value, label){
        var dd = ddWrap(input); if(!dd || !input){ return; }
        input.value = value;
        var valueEl = dd.querySelector('.ab-dropdown__value'); if(valueEl){ valueEl.textContent = label; }
        dd.querySelectorAll('.ab-dropdown__option').forEach(function(opt){
            if(opt.getAttribute('data-value') === value){ opt.classList.add('is-selected'); opt.setAttribute('aria-selected','true'); }
            else { opt.classList.remove('is-selected'); opt.removeAttribute('aria-selected'); }
        });
        input.dispatchEvent(new Event('change', { bubbles: true }));
    }
    function ddAddOption(input, value, label){
        var dd = ddWrap(input); if(!dd){ return; }
        var menu = dd.querySelector('.ab-dropdown__menu'); if(!menu){ return; }
        var li = document.createElement('li');
        li.className = 'ab-dropdown__option';
        li.setAttribute('role', 'option');
        li.setAttribute('data-value', value);
        li.textContent = label;
        menu.appendChild(li);
    }
    function ddRemoveOption(input, value){
        var dd = ddWrap(input); if(!dd){ return; }
        var opt = dd.querySelector('.ab-dropdown__option[data-value="' + value + '"]');
        if(opt){ opt.remove(); }
    }
    function ddRenameOption(input, value, label){
        var dd = ddWrap(input); if(!dd){ return; }
        var opt = dd.querySelector('.ab-dropdown__option[data-value="' + value + '"]');
    ...
    function ddFirstOption(input){
        var dd = ddWrap(input); if(!dd){ return null; }
        return dd.querySelector('.ab-dropdown__option');
    }
    function post(data){
    ...
    function loadRole(slug) {
        ...
        var sel=qs('#ab-role-select'), dispName=sel?(ddSelectedLabel(sel)||slug):slug;
    ...
    if(saveBtn){ saveBtn.addEventListener('click',function(){
        var name=ddSelectedLabel(qs('#ab-role-select'))||currentRole;
    ...
    // Inject/clear save spinner on a button. Bypasses admbudSaveStartLoading's
    // data-suppress-spinner check so we can drive the spinner manually for
    // this confirm-then-AJAX flow (the global capture-phase handler is
    // suppressed via data-suppress-spinner on the button so it doesn't get
    // stuck if the user cancels the confirm modal).
    function startSaveSpinner(b){
        if(!b || b.querySelector('.ab-spinner')) { return; }
        b.insertAdjacentHTML('afterbegin','<span class="ab-spinner ab-spinner--sm" aria-hidden="true" style="margin-right:6px;vertical-align:middle"></span>');
        b.disabled=true;
    }
    function endSaveSpinner(b){
        if(!b) { return; }
        var s=b.querySelector('.ab-spinner'); if(s){ s.parentNode.removeChild(s); }
    }
    function doSaveCaps(){
        var caps=qsa('.ab-cap-check:checked').map(function(c){return c.value;});
        var btn=qs('#ab-role-save-btn');
        var topbarBtn=qs('#ab-topbar-actions .ab-btn--save');
        ...
        post({action:'admbud_roles_save',nonce:nonce,role:currentRole,caps:JSON.stringify(caps)}).then(function(res){
            ...
            endSaveSpinner(btn); endSaveSpinner(topbarBtn);
            window.showToast(S.requestFailed||'Request failed.','error'); if(btn){btn.disabled=false;}
        });
    }
    ...
    if(delBtn){ delBtn.addEventListener('click',function(){
        window.openConfirmModal(S.roleDeleteConfirmTitle||'Delete Role?',S.roleDeleteConfirmBody||'Users will be moved to Subscriber. Cannot be undone.',function(){
            post({action:'admbud_roles_delete',nonce:nonce,role:currentRole}).then(function(res){
                if(res.success){
                    ddRemoveOption(roleSelect, currentRole);
                    delete roleCaps[currentRole];
                    var first=ddFirstOption(roleSelect);
                    // ddSetValue dispatches 'change' which calls loadRole - no need to call it again.
                    if(first){ ddSetValue(roleSelect, first.getAttribute('data-value')||'', first.textContent.trim()); }
    ...
    if(renameBtn){ renameBtn.addEventListener('click',function(){
        var inp=qs('#ab-rename-role-input'); if(inp){inp.value=ddSelectedLabel(qs('#ab-role-select'))||'';}
    ...
    if(renameConfirm){ renameConfirm.addEventListener('click',function(){
        ...
        post({action:'admbud_roles_rename',nonce:nonce,role:currentRole,name:name}).then(function(res){
            if (res.success) {
                ddRenameOption(roleSelect, currentRole, name);
    ...
    function openCreateModal(clone){
        ...
        var cf=qs('#ab-clone-from');
        if(cf){
            if(clone){
                var label = ddSelectedLabel(qs('#ab-role-select')) || currentRole;
                ddSetValue(cf, currentRole, label);
            } else {
                // Pull the "(none)" label from the dropdown's own option so the
                // translated PHP string is the source of truth.
                var noneOpt = ddWrap(cf) ? ddWrap(cf).querySelector('.ab-dropdown__option[data-value=""]') : null;
                var noneLabel = noneOpt ? noneOpt.textContent.trim() : '';
                ddSetValue(cf, '', noneLabel);
            }
        }
    ...
    if(createConfirm){ createConfirm.addEventListener('click',function(){
        ...
        post({action:'admbud_roles_create',nonce:nonce,name:name,clone_from:cf}).then(function(res){
            if(res.success){
                ...
                if(roleSelect){
                    ddAddOption(roleSelect, res.data.slug, res.data.name);
                    // ddSetValue fires 'change' -> loadRole, so no explicit loadRole here.
                    ddSetValue(roleSelect, res.data.slug, res.data.name);
                }
                hideModal('ab-create-role-modal'); window.showToast(S.roleCreated||'Role created.','success');
