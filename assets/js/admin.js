...
( function () {
    ...
    //
    // No leading icon — visual feedback during submit is the spinner
    // injected by `admbudSaveStartLoading()` below (mirrors EB's pattern).
    // Static save-floppy icon was dropped because (a) it competed with the
    // dirty-state pulse dot for the same `::before` slot and (b) once the
    // spinner replaces it on submit there's nothing to indicate, so the
    // resting state is cleaner without it.
    ...
    function syncTopbarSave() {
        ...
        btn.type = 'button';
        // `ab-btn--save` carries the compact-primary topbar dimensions.
        btn.className = 'ab-btn ab-btn--primary ab-btn--save';
        btn.textContent = label;
        btn.disabled = origBtn.disabled;
        if (!origBtn.disabled) { btn.classList.add('has-changes'); }
        // Inherit spinner opt-out so AJAX flows that drive their own loading
        // state aren't fighting the global capture-phase handler.
        if (origBtn.dataset.suppressSpinner) { btn.dataset.suppressSpinner = '1'; }
        btn.addEventListener('click', function () {
            admbudSubmitting = true;
            // Trigger the original button — its form's submit listener
            // (below) will inject the spinner into both buttons.
            origBtn.click();
        });
    ...
    // Mirrors EB's `ecobudSaveDirty` pattern. On form submit a spinner is
    // injected into the leading edge of the Save button(s) and the button
    // is disabled. For traditional submit-redirect flows the page reload
    // clears everything; AJAX flows MUST call
    // `window.admbudSaveDirty.endLoading( btn )` in their success/error
    // handlers to remove the spinner + re-enable the button.
    //
    // `data-suppress-spinner` on the button or its form opts out — useful
    // for callers that handle their own loading UX.
    function admbudSaveStartLoading( btn, form ) {
        if ( ! btn ) { return; }
        if ( btn.dataset.suppressSpinner ) { return; }
        if ( form && form.dataset && form.dataset.suppressSpinner ) { return; }
        if ( btn.querySelector( '.ab-spinner' ) ) { return; }
        btn.insertAdjacentHTML(
            'afterbegin',
            '<span class="ab-spinner ab-spinner--sm" aria-hidden="true" style="margin-right:6px;vertical-align:middle"></span>'
        );
        btn.disabled = true;
    }

    function admbudSaveEndLoading( btn ) {
        if ( ! btn ) { return; }
        var spin = btn.querySelector( '.ab-spinner' );
        if ( spin ) { spin.parentNode.removeChild( spin ); }
        btn.disabled = false;
    }

    // Auto-bind on CLICK (not submit) — the spinner needs to be in the DOM
    // *and painted* before the browser starts unloading the page for a
    // form-post-redirect. A `submit` listener races with navigation; the
    // mutation lands but the browser leaves before the next paint.
    // Click fires earlier in the chain (click → default action → submit
    // → unload), so inserting the spinner here gives the renderer a frame
    // to commit before navigation begins.
    //
    // Delegated to document so dynamically-added save buttons work too.
    // Idempotent — `admbudSaveStartLoading` skips if a spinner is already
    // present (e.g. when the topbar mirror's click handler triggers the
    // original button's click below, both fire the same handler).
    function admbudSpinnerOnClick( e ) {
        var btn = e.target.closest( '.ab-form-save-btn, .ab-btn--save' );
        if ( ! btn ) { return; }
        if ( btn.disabled ) { return; }
        var form = btn.form
            || ( btn.getAttribute( 'form' )
                ? document.getElementById( btn.getAttribute( 'form' ) )
                : null )
            || btn.closest( 'form' );
        admbudSaveStartLoading( btn, form );
        // Mirror to the topbar Save button if syncTopbarSave placed one
        // (so clicking the in-form button also lights the topbar mirror).
        var topbarSlot = document.getElementById( 'ab-topbar-actions' );
        if ( topbarSlot ) {
            var topbarBtn = topbarSlot.querySelector( '.ab-btn--save' );
            if ( topbarBtn && topbarBtn !== btn ) {
                admbudSaveStartLoading( topbarBtn, form );
            }
        }
        // Mirror to the in-form button when the topbar mirror is clicked
        // (the topbar btn's own click handler then triggers `origBtn.click()`,
        // but origBtn click bubbles up here — covered by the lookup above).
    }
    document.addEventListener( 'click', admbudSpinnerOnClick, true );

    window.admbudSaveDirty = {
        startLoading: admbudSaveStartLoading,
        endLoading:   admbudSaveEndLoading
    };
    ...
    // Save Changes — direct submit, no confirmation modal.
    //
    // Earlier this opened an `openConfirmModal('Save changes?', ...)` dialog
    // before submitting. Retired because:
    //   - Save is the highest-frequency action; one extra click per save
    //     compounds across the lifetime of the UI.
    //   - The modal-on-Save flow conflicts with the spinner injector
    //     (`admbudSpinnerOnClick` at top of file): the spinner starts on
    //     click, then the modal opens *over* it, and if the user cancels
    //     the modal there's no clean path to clear the spinner — leaving
    //     it stuck. EB has no save-confirmation modal and the UX is
    //     visibly cleaner.
    //   - Toast on success ("Settings saved") + the dirty-pulse dot give
    //     enough feedback for a non-destructive operation.
    // Confirmation modals are kept for genuinely destructive flows (Reset,
    // Delete Plugin Data, Import Settings, etc.) where the consequences
    // are irreversible.
    on( document, 'click', '.ab-form-save-btn', function(e) {
        ...
        // for any reset-state pickers (preserves the cleanup logic that
        // previously lived inside the modal callback).
    ...
    // May 2026 because the iframe approach forced an inline <style> or
    // <link rel="stylesheet"> inside the srcdoc, both of which Plugin Check
    // flags). Dynamic values flow via CSS custom properties on the wrapper
    // div + textContent of the title/message children.
    function applyMaintPreview( wrapper, bg, headingColor, msgColor, title, message ) {
        if ( ! wrapper ) { return; }
        wrapper.style.setProperty( '--ab-preview-bg',      bg );
        wrapper.style.setProperty( '--ab-preview-heading', headingColor );
        wrapper.style.setProperty( '--ab-preview-message', msgColor );
        var t = wrapper.querySelector( '.ab-maint-preview__title' );
        var m = wrapper.querySelector( '.ab-maint-preview__msg' );
        if ( t ) { t.textContent = title; }
        if ( m ) { m.textContent = message; }
    }

    function applyMaintPreviewEmpty( wrapper ) {
        if ( ! wrapper ) { return; }
        applyMaintPreview( wrapper, '#f1f5f9', '#94a3b8', '#94a3b8', '', 'Enable a mode to see preview.' );
    }

    function updateMaintPreview() {
        var wrapper = qs( '#ab-maint-page-preview' );
        if ( ! wrapper ) { return; }

        var mode = ( qs( 'input[name="admbud_maintenance_mode"]:checked' ) || {} ).value || 'off';
        if ( mode === 'off' ) {
            applyMaintPreviewEmpty( wrapper );
            return;
        }
    ...
    on( document,'change','.ab-setup-tab-toggle', function() {
        ...
        var fd=new FormData();
        fd.append('action','admbud_modules_toggle'); fd.append('nonce',nonce);
