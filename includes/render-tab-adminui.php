            ...
            <div class="ab-subtab-layout">
                <div class="ab-subtab-main">
                    ...
                    <div class="ab-pane<?php echo $active !== 'adminui-dashboard' ? ' ab-hidden' : ''; ?>" id="ab-pane-adminui-dashboard">
                        ...
                        <?php
                        // -- Card: Dashboard Page ----------------------------------
                        // Helper: emit a single page row inside the styled-select
                        // dropdown menu (mirrors EB's ecobud-dropdown markup).
                        // `$selected` is the currently-stored value; the matching
                        // page row gets `.is-selected` so the open menu shows
                        // which page is current. Non-published pages render with
                        // a status suffix so the user can tell drafts/private
                        // apart at a glance.
                        /*
                         * Closure ECHOES rather than returns so callsites can call
                         * it as a statement (no outer echo). Plugin Check / WP.org
                         * reviewers don't introspect helpers — keeping the echo
                         * inside the helper means there is no flagable
                         * echo-of-helper-return at the call sites.
                         *
                         * NOTE: Do not write `<?` `php ... ?` `>` inside `//`
                         * comments here — PHP's single-line comment is terminated
                         * by a literal `?` followed by `>` even *inside* the
                         * comment text, which would close the surrounding PHP
                         * block and prevent the assignment below from running.
                         * Use block comments for any prose that mentions PHP
                         * delimiters.
                         */
                        $admbud_page_dropdown_lis = function( $pages, $selected ) {
                            foreach ( $pages as $p ) {
                                $label = $p->post_title ?: __( '(no title)', 'admin-buddy' );
                                if ( $p->post_status !== 'publish' ) {
                                    $label .= ' [' . ucfirst( $p->post_status ) . ']';
                                }
                                $is_selected = ( (string) $p->ID === (string) $selected );
                                printf(
                                    '<li class="ab-dropdown__option%s" role="option" data-value="%s"%s>%s</li>',
                        ...
                        // Resolve the current default-page label so the trigger
                        // text reflects what's stored. `0` is "WP dashboard"; any
                        // other value matches a page ID.
                        $admbud_default_page_label = __( '- WordPress Dashboard -', 'admin-buddy' );
                        if ( (int) $default_page > 0 ) {
                            foreach ( $pages as $p ) {
                                if ( (int) $p->ID === (int) $default_page ) {
                                    $admbud_default_page_label = $p->post_title ?: __( '(no title)', 'admin-buddy' );
                                    break;
                                }
                            }
                        }
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="ab-dropdown ab-dropdown--select">
                                        <input type="hidden"
                                               id="admbud_dashboard_default_page"
                                               class="ab-dashboard-page-select"
                                               data-role="_default"
                                               data-ab-dropdown-input
                                               value="<?php echo esc_attr( $default_page ); ?>">
                                        <button type="button" class="ab-dropdown__trigger" aria-haspopup="listbox" aria-expanded="false">
                                            <span class="ab-dropdown__value"><?php echo esc_html( $admbud_default_page_label ); ?></span>
                                            <span class="ab-dropdown__caret" aria-hidden="true">▾</span>
                                        </button>
                                        <ul class="ab-dropdown__menu" role="listbox" hidden>
                                            <li class="ab-dropdown__option<?php echo (int) $default_page === 0 ? ' is-selected' : ''; ?>"
                                                role="option"
                                                data-value="0"
                                                <?php if ( (int) $default_page === 0 ) echo 'aria-selected="true"'; ?>>
                                                <?php esc_html_e( '- WordPress Dashboard -', 'admin-buddy' ); ?>
                                            </li>
                                            <?php $admbud_page_dropdown_lis( $pages, $default_page ); ?>
                                        </ul>
                                    </div>
                        ...
                        <div id="ab-dashboard-role-grid" class="<?php echo $has_role_overrides ? '' : 'ab-hidden'; ?>" style="margin-top:var(--ab-space-2);">
                            <div class="ab-grid ab-grid--ruled ab-grid--module">
                                ...
                                ?>
                                <?php
                                // Resolve trigger label: defer label, wp_default
                                // label, or page title — match the stored value.
                                $admbud_role_trigger_label = __( 'Use Default', 'admin-buddy' );
                                if ( $role_page === 'wp_default' ) {
                                    $admbud_role_trigger_label = __( 'WordPress Dashboard', 'admin-buddy' );
                                } elseif ( $role_page !== '' && (int) $role_page > 0 ) {
                                    foreach ( $pages as $p ) {
                                        if ( (int) $p->ID === (int) $role_page ) {
                                            $admbud_role_trigger_label = $p->post_title ?: __( '(no title)', 'admin-buddy' );
                                            break;
                                        }
                                    }
                                }
                                ...
                                <div class="ab-setup-module-row" style="flex-direction:column;align-items:stretch;gap:var(--ab-space-1);">
                                    ...
                                    <div class="ab-dropdown ab-dropdown--select">
                                        <input type="hidden"
                                               ...
                                               data-ab-dropdown-input
                                               value="<?php echo esc_attr( $role_page ); ?>">
                                        <button type="button" class="ab-dropdown__trigger" aria-haspopup="listbox" aria-expanded="false">
                                            <span class="ab-dropdown__value"><?php echo esc_html( $admbud_role_trigger_label ); ?></span>
                                            <span class="ab-dropdown__caret" aria-hidden="true">▾</span>
                                        </button>
                                        <ul class="ab-dropdown__menu" role="listbox" hidden>
                                            <li class="ab-dropdown__option<?php echo $role_page === '' ? ' is-selected' : ''; ?>"
                                                role="option"
                                                data-value=""
                                                <?php if ( $role_page === '' ) echo 'aria-selected="true"'; ?>>
                                                <?php esc_html_e( 'Use Default', 'admin-buddy' ); ?>
                                            </li>
                                            <li class="ab-dropdown__option<?php echo $role_page === 'wp_default' ? ' is-selected' : ''; ?>"
                                                role="option"
                                                data-value="wp_default"
                                                <?php if ( $role_page === 'wp_default' ) echo 'aria-selected="true"'; ?>>
                                                <?php esc_html_e( 'WordPress Dashboard', 'admin-buddy' ); ?>
                                            </li>
                                            <?php $admbud_page_dropdown_lis( $pages, $role_page ); ?>
                                        </ul>
                                    </div>
                                </div>
                                <?php endforeach; ?>
