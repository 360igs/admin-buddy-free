...
class Colours {
    ...
    //
    // **Default preset slug** — when the Colours module is enabled on a fresh
    // install, the option values seeded by Settings::defaults() match the
    // preset named here. To change AB's out-of-the-box palette, change this
    // slug AND make sure the corresponding values in Settings::defaults()
    // match the new preset's `values` array in Settings::get_colour_presets().
    // (A future refactor could derive Settings::defaults() programmatically
    // from this slug — left as a TODO to keep the WP.org resubmit scope small.)
    //
    // The constants below (DEFAULT_PRIMARY etc.) are now used only as PHP-side
    // fallbacks when an option read returns empty (e.g. after a reset). The
    // admin chrome itself no longer falls back to these — when the Colours
    // module is *off*, [tokens.css]'s --ab-primary falls back to WordPress
    // core's --wp-admin-theme-color (the user's chosen admin colour scheme),
    // not to AB's brand purple. Keep the constants in sync with the named
    // default preset so the two layers don't diverge.
    const DEFAULT_PRESET_SLUG       = 'violety';
    ...
    private function build_admin_css( bool $excluded = false ): void {
        ...
        echo 'opacity: 1 !important;';
        echo '}';
        // nav-tab-active: do NOT override background/border/text. The active-tab
        // fill used to take --ab-menu-bg, which painted third-party plugin
        // settings tabs (WooCommerce, etc.) with AB's dark sidebar colour and
        // read as a colour bleed against their light content area. Native
        // active-tab styling blends into the page bg — let that stand.
        ...
        // Only theme the :hover text colour. The active-tab fill was previously
        // forced to var(--ab-menu-bg) (dark sidebar colour), which painted
        // third-party plugin settings tabs — notably WooCommerce → Settings —
        // with a dark block over their light content area (a visible colour
        // bleed). Native active-tab styling already merges with the page bg,
        // so leave it alone.
        if ( ! $excluded ) {
            $admbud_css = '#wpcontent .wrap .nav-tab-wrapper .nav-tab:hover{color:var(--ab-primary)!important;}';
