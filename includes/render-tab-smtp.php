...
<form method="post" action="options.php" id="ab-smtp-form">
    ...
    <table class="form-table ab-form-table" role="presentation">
        ...
        <tr>
            ...
            <td>
                <?php
                $admbud_smtp_enc_labels = [
                    'tls' => 'TLS',
                    'ssl' => 'SSL',
                    ''    => __( 'None', 'admin-buddy' ),
                ];
                $admbud_smtp_enc_current = $admbud_smtp_enc_labels[ $encryption ] ?? $admbud_smtp_enc_labels[''];
                ?>
                <div class="ab-dropdown ab-dropdown--select">
                    <input type="hidden" name="admbud_smtp_encryption" id="admbud_smtp_encryption" class="ab-select" data-ab-dropdown-input value="<?php echo esc_attr( $encryption ); ?>">
                    <button type="button" class="ab-dropdown__trigger" aria-haspopup="listbox" aria-expanded="false">
                        <span class="ab-dropdown__value"><?php echo esc_html( $admbud_smtp_enc_current ); ?></span>
                        <span class="ab-dropdown__caret" aria-hidden="true">▾</span>
                    </button>
                    <ul class="ab-dropdown__menu" role="listbox" hidden>
                        <?php foreach ( $admbud_smtp_enc_labels as $admbud_enc_val => $admbud_enc_label ) : ?>
                            <li class="ab-dropdown__option<?php echo $encryption === $admbud_enc_val ? ' is-selected' : ''; ?>"
                                role="option"
                                data-value="<?php echo esc_attr( $admbud_enc_val ); ?>"
                                <?php if ( $encryption === $admbud_enc_val ) echo 'aria-selected="true"'; ?>>
                                <?php echo esc_html( $admbud_enc_label ); ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
...
<?php else : ?>
    ...
    <table class="widefat striped ab-smtp-log-table">
        ...
        ?>
            <tr class="ab-log-row" data-entry="<?php echo esc_attr( wp_json_encode( $entry ) ); ?>" style="cursor:pointer;" title="<?php esc_attr_e( 'Click to view full email', 'admin-buddy' ); ?>">
