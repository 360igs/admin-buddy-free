...
<div class="ab-roles-wrap">
    ...
    <div class="ab-toolbar ab-roles-toolbar">
        ...
        <div class="ab-toolbar__left ab-roles-toolbar__left">
            <label class="ab-toolbar__label ab-roles-toolbar__label">
            ...
            <?php
            // First role wins as the initial selection (matches the native
            // <select>'s default-first-option behaviour the JS relied on).
            $admbud_roles_first_slug  = $all_roles ? array_key_first( $all_roles ) : '';
            $admbud_roles_first_label = $admbud_roles_first_slug ? ( $all_roles[ $admbud_roles_first_slug ]['name'] ?? $admbud_roles_first_slug ) : '';
            ?>
            <div class="ab-dropdown ab-dropdown--select ab-roles-role-select-wrap">
                <input type="hidden" id="ab-role-select" class="ab-select ab-roles-role-select" data-ab-dropdown-input value="<?php echo esc_attr( $admbud_roles_first_slug ); ?>">
                <button type="button" class="ab-dropdown__trigger" aria-haspopup="listbox" aria-expanded="false">
                    <span class="ab-dropdown__value"><?php echo esc_html( $admbud_roles_first_label ); ?></span>
                ...
                <ul class="ab-dropdown__menu" role="listbox" hidden>
                    <?php foreach ( $all_roles as $slug => $role ) : ?>
                        <li class="ab-dropdown__option<?php echo $slug === $admbud_roles_first_slug ? ' is-selected' : ''; ?>"
                            role="option"
                            data-value="<?php echo esc_attr( $slug ); ?>"
                            <?php if ( $slug === $admbud_roles_first_slug ) echo 'aria-selected="true"'; ?>>
    ...
    <div class="ab-topbar-actions-mirror" style="display:none;">
        <button type="button" id="ab-role-save-btn" class="ab-btn ab-btn--primary" data-suppress-spinner disabled>
    ...
    <div class="ab-modal__box">
        ...
        <div class="ab-modal__fields">
            ...
            <div id="ab-clone-from-row" class="ab-field-row" style="display:none;">
                <label class="ab-field-label"><?php esc_html_e( 'Copy caps from', 'admin-buddy' ); ?></label>
                <div class="ab-dropdown ab-dropdown--select">
                    <input type="hidden" id="ab-clone-from" class="ab-select" data-ab-dropdown-input value="">
                    <button type="button" class="ab-dropdown__trigger" aria-haspopup="listbox" aria-expanded="false">
                        <span class="ab-dropdown__value"><?php esc_html_e( '- None (start blank) -', 'admin-buddy' ); ?></span>
                        <span class="ab-dropdown__caret" aria-hidden="true">▾</span>
                    </button>
                    <ul class="ab-dropdown__menu" role="listbox" hidden>
                        <li class="ab-dropdown__option is-selected" role="option" data-value="" aria-selected="true">
                            <?php esc_html_e( '- None (start blank) -', 'admin-buddy' ); ?>
                        </li>
                        <?php foreach ( $all_roles as $slug => $role ) : ?>
                            <li class="ab-dropdown__option" role="option" data-value="<?php echo esc_attr( $slug ); ?>">
                                <?php echo esc_html( $role['name'] ); ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
