...
class Maintenance {
    ...
    private function render_page( string $mode ): void {
        ...
        // sanitiser. Every interpolation is esc_attr()'d at substitution time.
        $admbud_maint_css = '*, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }'
            . 'body {'
            ...
            . '}'
            . '.ab-page { text-align: center; max-width: 520px; width: 100%; }'
            . '.ab-page__title  { font-size: 2rem; font-weight: 800; margin-bottom: 1rem; color: ' . esc_attr( $heading_color ) . '; line-height: 1.2; }'
            . '.ab-page__message { font-size: 1.05rem; line-height: 1.75; color: ' . esc_attr( $message_color ) . '; }';

        // Use the wp_enqueue API even though we render outside the normal head
        // pipeline (this is a die()-served self-contained HTML page). Register a
        // virtual handle (false src), attach the inline body, then flush via
        // wp_print_styles() inside the <head> echoes. This satisfies WP.org's
        // "no echo of literal <style> tag" rule while preserving the existing
        // intercept-page rendering contract.
        wp_register_style( 'admbud-maint-inline', false, [], ADMBUD_VERSION );
        wp_add_inline_style( 'admbud-maint-inline', $admbud_maint_css );
        ...
        wp_print_styles( [ 'admbud-maint-inline' ] );
        echo '</head>';
        echo '<body>';
