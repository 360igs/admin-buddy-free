...
trait Settings_Render {
    ...
    /** Open a settings card with an inline SVG icon. SVG sanitised via wp_kses() with the AB SVG ruleset. */
    public function card_open_svg( string $svg, string $title, string $subtitle = '' ): void {
        ?>
        <div class="ab-section">
            <div class="ab-section__header">
                <span class="ab-section__icon ab-section__icon"><?php echo wp_kses( $svg, admbud_kses_svg_allowed() ); ?></span>
                <div>
                    <h3><?php echo esc_html( $title ); ?></h3>
                    <?php if ( $subtitle ) : ?><p><?php echo esc_html( $subtitle ); ?></p><?php endif; ?>
                </div>
     ...
     *
     * Was previously an iframe with `srcdoc`, which forced the preview's CSS
     * to ship as either an inline <style> block or a <link rel="stylesheet">
     * inside the srcdoc — Plugin Check flags both because there's no
     * wp_enqueue context inside an iframe srcdoc. Switched to a styled <div>
     * (May 2026): static styling lives in admin.css under .ab-maint-preview,
     * dynamic colours come in as CSS custom properties on the wrapper. No
     * iframe, no enqueue conflict, simpler live-update path.
     *
     * @param string $prefix    'cs' or 'maint' - matches the DB option prefixes.
     * @param string $iframe_id  DOM id (legacy parameter name kept for caller
     *                           compatibility; applied to the wrapper <div>).
     */
    public function render_page_preview( string $prefix, string $iframe_id = '' ): void {
        ...
        $message_text = admbud_get_option(
            $is_maint ? 'admbud_maintenance_message' : 'admbud_coming_soon_message',
            __( 'We\'ll be back shortly.', 'admin-buddy' )
        );

        $preview_styles = '--ab-preview-bg:' . $initial_bg . ';'
            . '--ab-preview-heading:' . $heading_c . ';'
            . '--ab-preview-message:' . $message_c . ';';
        $preview_title = $is_maint
            ? __( 'Maintenance page preview', 'admin-buddy' )
            : __( 'Coming soon page preview', 'admin-buddy' );
        ?>
        <div
            ...
            <div class="ab-maint-preview__card">
                <div class="ab-maint-preview__title"><?php echo esc_html( $heading_text ); ?></div>
                <div class="ab-maint-preview__msg"><?php echo esc_html( $message_text ); ?></div>
            </div>
        </div>
