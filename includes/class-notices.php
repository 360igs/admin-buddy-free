...
class Notices {
    ...
    private function is_plugin_callback( $callable ): bool {
        ...
        // categorise where a notice callback was defined. WP's URL helpers
        // don't apply here - we need actual disk-path roots for prefix
        // matching. Values are normalised + trailing-slashed.
        $file = wp_normalize_path( $file );

        // Keep anything from core (wp-includes or wp-admin).
        ...
        // Keep callbacks from the active theme (parent + child) and from
        // mu-plugins (local infrastructure, never spam).
        ...
        // Anything else: is the file in the regular plugins root? Core's
        // plugin_basename() returns a path different from the normalised
        // input when the file lives under the plugins directory (and also
        // handles symlinked plugin realpaths via $wp_plugin_paths). MU-plugin
        // files were kept above, so reaching this point with plugin_basename()
        // stripping a prefix means a regular plugin.
        return plugin_basename( $file ) !== $file;
    }
